...
class ChannelAttention(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        avg_out = avg_out.view(avg_out.size(0), -1)  # Flatten
        avg_out = self.fc(avg_out).view(avg_out.size(0), avg_out.size(1), 1, 1)
        return x * avg_out  # Apply channel-wise attention
...
class SpatialAttention(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        out = torch.cat([avg_out, max_out], dim=1)  # Concatenate along channel dimension
        out = self.conv(out)
        out = self.sigmoid(out)
        return x * out  # Apply spatial attention


# Gram Matrix for Feature Alignment
class GramMatrix(nn.Module):
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        b, c, h, w = x.size()
        features = x.view(b, c, -1)
        gram_matrix = torch.bmm(features, features.transpose(1, 2))  # Compute Gram matrix
        gram_matrix = gram_matrix / (c * h * w)  # Normalize by the number of elements
        return gram_matrix


# MPKD-DCFI Model: Incorporating Channel Attention, Spatial Attention, and Gram Matrix
class MPKD_DCFI_Model(nn.Module):
    def __init__(self, num_classes: int, in_channels: int, reduction_ratio: int = 16, kernel_size: int = 7):
        super(MPKD_DCFI_Model, self).__init__()
        self.channel_attention = ChannelAttention(in_channels, reduction_ratio)
        self.spatial_attention = SpatialAttention(kernel_size)
        self.gram_matrix = GramMatrix()

        # Define teacher and student models
        self.teacher_model = get_model("ResNet20", num_classes)  # Replace with your model
        self.student_model = get_model("ResNet56", num_classes)  # Replace with your model
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        student_features = self.student_model(x)

        # Compute Gram matrix for feature alignment
        teacher_gram = self.gram_matrix(teacher_features)
        student_gram = self.gram_matrix(student_features)

        # Compute distillation loss (cross-entropy, KD, or other custom losses here)
        distillation_loss = self.compute_distillation_loss(student_gram, teacher_gram)

        return distillation_loss

    def compute_distillation_loss(self, student_gram, teacher_gram):
        # Compute Gram loss (you could use MSE or other distance metrics)
...
def train_epoch(
       ...
       # Training for one epoch with MPKD-DCFI model, optimizing the distillation loss
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        optimizer.zero_grad()
        loss = module_dict["student"](data)
        loss.backward()
        optimizer.step()

        tb_writer.add_scalar("train/loss", loss.item(), epoch)

        # Optionally, you can compute and log additional metrics here (accuracy, etc.)
    ...
    for idx, data in enumerate(train_loader):
        ...
        action = gumbel_softmax(policy_res.view(policy_res.size(0), -1, 2), temperature=3)
...
def train_kd(
    ...
    for epoch in range(1, cfg["training"]["epochs"] + 1):
        ...
        a = format(val_acc, '.4f')
        b = format(val_loss,'.4f')
        list = [epoch, a,b]
        data = pd.DataFrame([list])
        data.to_csv('E:\\spot-adaptive-pytorch-maintest -two\\spot-adaptive-pytorch-main\\tiny-image-val_acc1+loss.csv', mode='a', header=False, index=False)
...
def main(
    ...
    # get dataloaders
    logger.info("Loading datasets... ",cfg["kd_loss"]["name"])
    if (cfg["kd_loss"]["name"] == 'CRD' )and (cfg["dataset"]["name"] in ["cifar100", "CIFAR100", "cifar-100"]):
    ...
    if cfg["kd_loss"]["name"] in ['AT', 'NST', 'VID', 'KDSVD', 'AB']:
        ...
        policy = LinearClassifier(model_t.get_hint_channel() + model_s.get_hint_channel(), real_len * 5).to(device)
...
if __name__ == "__main__":
    ...
    # parser.add_argument("--config", type=str, default='configs/cifar-100/seed-1029/at/vgg13-vgg8.yml')
    # parser.add_argument("--logdir", type=str, default='run/cifar-100/seed-1029/if(r20,3,2)+temp=3/vgg13-vgg8')
    # # parser.add_argument("--file_name_cfg", type=str, default='kd-vgg13-vgg8-cifar100-debug')
    #
    # parser.add_argument("--file_name_cfg", type=str, default='kd-vgg13-vgg8-cifar100-if(r20,3,2)+temp=3')
    # # parser.add_argument("--file_name_cfg", type=str, default='kd-vgg13-vgg8-cifar100-if(r20,3,2)+bias10+20+10+loss1')

    parser.add_argument("--config", type=str, default='configs/tiny-imagenet/seed-1029/at/vgg13-vgg8.yml')

    parser.add_argument("--logdir", type=str, default='run/tiny-imagenet/seed-1029/vgg13-vgg8')
    # parser.add_argument("--file_name_cfg", type=str, default='kd-vgg13-vgg8-cifar100-debug')

    parser.add_argument("--file_name_cfg", type=str, default='kd-vgg13-vgg8-tiny-imagenet-vgg13-vgg8')
